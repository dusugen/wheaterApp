...
const WeatherView: FC<WeatherViewProps> = ({ weatherData }) => (
  <Box>
    ...
    <Center mt={normalize(60)}>
      <Text>
        Weather on
        {moment(weatherData.list[0].dt_txt).format('DD-MM-YYYY')}
      </Text>
      <Box display="flex" flexDirection="row" marginTop={normalize(20)}>
        <Heading size="4xl">{Math.round(weatherData.list[0].main.temp)}</Heading>
